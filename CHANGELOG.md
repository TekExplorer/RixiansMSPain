# Changelog

All notable changes to this project will be documented in this file.

## [v0.3.6] - 2026-09-19

- Fixed a bug where some cards with alternate arts that depend on the card's owner existing throwing errors in the compendium

## [v0.3.5] - 2026-09-16

- Resolve Necrobinder splash credit. No clipping!
- Fix Neurosurge bug on Main branch

## [v0.3.5-canary] - 2026-09-16

- Resolve Necrobinder splash credit. No clipping!
- Fix Neurosurge bug on Main branch

## [v0.3.4] - 2026-09-03

- Add missing The Ball art
- Fix Soulbound credit

## [v0.3.3] - 2026-09-03

- Updated credits
- Fixed Dowsing and Melancholy showing alt art indicators when not actually an alt art
- Fix Charge showing 0 draw alt art in card rewards
- Fixed issue with public branch players not being able to play multiplayer with people without the mod
- Enabled certain card effects on public branch

## [v0.3.2+2-canary] - 2026-09-02

- Fix it for real, cause i did, in fact, miss a thing.

## [v0.3.2+1-canary] - 2026-09-02

- Fix Main branch multiplayer issue, reupload

## [v0.3.2-canary] - 2026-09-02

- Fix Main branch multiplayer issue

## [v0.3.1-canary] - 2026-09-02

- Update credits
- Fixed Dowsing and Melancholy showing alt art indicators when not actually an alt art
- Fix Charge showing 0 draw alt art in card rewards

## [v0.3.0] - 2026-09-01

- Added Necrobinder, Ancient, and Event cards!
- And Character Select Screens!
- And a whole lot of cool interactions!
- And big preformance improvements!
- And more config options!
- Thank you for your patience!

## [v0.2.16-canary] 2026-09-01
- Fixed Right Hand Hand
- Added preloading so card arts don't cause lag when they load for the first time

## [v0.2.15-canary] - 2026-08-31
- Fixed Pull Aggro not showing its animation due to BaseLib bug
- Fixed credits showing as for upgraded art even when there is no distinct art for that card's upgrade.

## [v0.2.14-canary] - 2026-08-31
- Fixed Luminesce sparkles leaking to other cards
- Made more things responsive to the safe mode toggle
- Fixed Defile Exhaust icon being broken
- Added credits for sfx and vfx
- Switched to Atlases for card images, dramatically improving loading in the compendium
- Added some padding to the top and left of the arts so that everything is properly aligned (goddammit Mega Crit)
- Shifted Pull Aggro to be better aligned
- Fixed a bug with Infinite Blades VFX

## [v0.2.13-canary] - 2026-08-28

- Fixed freddy epitaph appearing on new Soul
- Fixed credits not showing when tooltips are enabled
- Updated Right Hand Hand
- Updated Death's Door
- Added base/canonical Mad Science (Power) just in case
- Fixed bug where Shared Fate would break in the compendium
- Fixed issue where canary wouldnt use the inverted mod image

## [v0.2.12-canary] - 2026-08-27

- Added Neurosurge Yippee!
- Added simple Squeeze effect
- Added creature effects for Squeeze, Flatten, and Rattle
- Added Squeeze+, Reflect+, and updated arts for Reflect, Squeeze, The Bomb, and Skewer
- Canary-only arts can now be visually shown to non-canary users.
- Added a Safe mode option to disable special effects in case something breaks
- Added Main branch compatibility
- Added In-game Credits!
- Fixed compendium bugs with shiv and soul canary arts
- Fixed art loading bug

## [v0.2.11-canary] - 2026-08-01

- Added Cacophony
- Illuminated Luminesce
- Fixed beta v0.110 compatibility
- Made some things hopefully more reliable and less likely to break
- Added special code for multiplayer viewing of certain cards, and added a command to help debug it!
- Subsequently applied that to Shiv and Soul using a WIP art

## [v0.2.10-canary] - 2026-07-24

- Attempted to fix Parry bug
- Added Putrefy animation

## [v0.2.9-canary] - 2026-07-23

- Added credits
- Added Rebound, Spirit of Ash+.
- Added Deathbringer+ and un-ungraded Neurosurge
- Added Piercing Wail alt art

## [v0.2.8-canary] - 2026-07-21

- Canary now has a unique mod image.
- Added Defile exhaust pile icon.
- Added Underworld
- Added Clash effect
- Added Osty dance! (a little scuffed)

## [v0.2.7-canary] - 2026-07-20

- Added abundance, plot, and demesne alt art. Added freddy epitaph to cleanse

## [v0.2.6-canary] - 2026-07-20

- Added Necrobinder, Quest, Ancient, and some beta cards. Some cards have been updated. Many Alt arts are in!

## [v0.2.5] - 2026-07-10

- Fixed Multiplayer issue. You can now play the mod without requiring the rest of the party to have it!

## [v0.2.4] - 2026-07-08

- Fix some credits
- Fix a bug with Glow's Sparkles showing on future non-Glow card previews
- Made the title banner a separate setting so it doesn't block the top of the art.

## [v0.2.3] - 2026-07-07

- Update Skewer, Reflect, and The Bomb with signed versions of their art

## [v0.2.2] - 2026-07-07

- Update Debris *again*

## [v0.2.1] - 2026-07-07

- Updated Credits and Debris art

## [v0.2.0] - 2026-07-07

- Added Curses, Statuses, and Regent cards!
- And their credits!
- And a bunch of alt arts!
- Made Infinite Blades more infinite

## [v0.1.18-beta] - 2026-07-06

- Attempt to fix Perry the Sovereign Blade

## [v0.1.17-beta] - 2026-07-06

- Add Spoils of Battle+ and alt art

## [v0.1.16-beta] - 2026-07-06

- Add Parry/Perry alt art

## [v0.1.15-beta] - 2026-07-06

- Calculated Gamble+, CG No Draw, and CG No Draw+
- Outbreak and Noxious Fumes should react to the power more immediately now

## [v0.1.14-beta] - 2026-07-06

- Unaligned Non-Alignment cards after Alignment aligned

## [v0.1.13-beta] - 2026-07-06

- Glowier Glow in more Glowy places!

## [v0.1.12-beta] - 2026-07-06

- Aligned Alignment
- Made Glow Glowier

## [v0.1.11-beta] - 2026-07-06

- Added Monologue Alt art
- Added the Outbreak half of the Outbreak-Noxious synergy
- Mind Rot'd Regent
- Updated Sucker Punch's art
- Added know_thy_place_plus
- Added forgone_conclusion_plus

## [v0.1.10-beta] - 2026-07-06

- Made the beta version easy to tell in the mod list
- Fixed some credits
- Attempted to add more discord id to display name mappings
- Made Infinite Blades more infinite

## [v0.1.9-beta] - 2026-07-06

- Regent cards!

## [v0.1.8] - 2026-07-03

- Add Curses/Statuses
- MISSED A COUPLE
- MAYBE WITHER???

## [v0.1.5] - 2026-07-02

- Update Abrasive
- Add Abrasive+ alt art
- Add Noxious Fumes alt art
- Fix the description visibility not working when re-enabled

## [v0.1.4] - 2026-07-02

- Add Accelerant alt art. Why no poison?

## [v0.1.3] - 2026-07-01

- Maybe don't let the infection tooltip leak into the game

## [v0.1.2] - 2026-07-01

- Even more infinitely infinite blades added!

## [v0.1.1] - 2026-06-29

- Update a couple credit entries
- Update a couple more credit entries

## [v0.1.0] - 2026-06-29

- Added in-game credits on all the cards!

## [v0.0.9] - 2026-06-28

- Replace shiv art with a new version due to complaints for some reason... idfk
- Also made the settings menu a bit fancier

## [v0.0.8] - 2026-06-27

- Fix localization bug

## [v0.0.7] - 2026-06-27

- Add colorless card arts

## [v0.0.6] - 2026-06-25

- Added an Easter Egg

## [v0.0.5] - 2026-06-24

- Fixed the Wither title issue properly, and removed the config in the process.
- Also properly hides card names in Multiplayer

## [v0.0.4] - 2026-06-23

- Added settings to optionally exclude Wither or Frantic Escape in order to see upgrade count or energy count respectively

## [v0.0.3] - 2026-06-23

- Attempt to fix a bug...?
- Fixed Shiv

## [v0.0.2] - 2026-06-23

- Added Silent cards

## [v0.0.1] - 2026-06-19

- Now shows when a card is upgraded
- Now hides the reward screen rarity glow.
- Added config items for many of the removed card elements so that you can turn them on for testing or if you just wanted the full-art.
- Added HideCardRewardRarityGlow config option

[v0.0.1]: https://github.com/TekExplorer/RixiansMSPain/releases/tag/v0.0.1








[v0.0.2]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.0.1...v0.0.2








[v0.0.3]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.0.2...v0.0.3








[v0.0.4]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.0.3...v0.0.4








[v0.0.5]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.0.4...v0.0.5








[v0.0.6]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.0.5...v0.0.6








[v0.0.7]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.0.6...v0.0.7








[v0.0.8]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.0.7...v0.0.8








[v0.0.9]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.0.8...v0.0.9








[v0.1.0]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.0.9...v0.1.0








[v0.1.1]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.0...v0.1.1








[v0.1.2]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.1...v0.1.2








[v0.1.3]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.2...v0.1.3








[v0.1.4]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.3...v0.1.4








[v0.1.5]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.4...v0.1.5








[v0.1.8]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.5...v0.1.8








[v0.1.9-beta]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.8...v0.1.9-beta








[v0.1.10-beta]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.9-beta...v0.1.10-beta








[v0.1.11-beta]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.10-beta...v0.1.11-beta








[v0.1.12-beta]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.11-beta...v0.1.12-beta








[v0.1.13-beta]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.12-beta...v0.1.13-beta








[v0.1.14-beta]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.13-beta...v0.1.14-beta








[v0.1.15-beta]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.14-beta...v0.1.15-beta








[v0.1.16-beta]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.15-beta...v0.1.16-beta








[v0.1.17-beta]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.16-beta...v0.1.17-beta








[v0.1.18-beta]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.17-beta...v0.1.18-beta








[v0.2.0]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.8...v0.2.0








[v0.2.1]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.0...v0.2.1








[v0.2.2]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.1...v0.2.2








[v0.2.3]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.2...v0.2.3








[v0.2.4]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.3...v0.2.4








[v0.2.5]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.4...v0.2.5








[v0.2.6-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.1.18-beta...v0.2.6-canary








[v0.2.7-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.6-canary...v0.2.7-canary








[v0.2.8-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.7-canary...v0.2.8-canary








[v0.2.9-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.8-canary...v0.2.9-canary








[v0.2.10-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.9-canary...v0.2.10-canary








[v0.2.11-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.10-canary...v0.2.11-canary








[v0.2.12-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.11-canary...v0.2.12-canary








[v0.2.13-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.12-canary...v0.2.13-canary








[v0.2.14-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.13-canary...v0.2.14-canary








[v0.2.15-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.14-canary...v0.2.15-canary








[v0.2.16-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.15-canary...v0.2.16-canary








[v0.3.0]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.2.5...v0.3.0








[v0.3.1-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.3.0...v0.3.1-canary







[v0.3.2-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.3.0...v0.3.2-canary






[v0.3.2+1-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.3.0...v0.3.2+1-canary





[v0.3.2+2-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.3.0...v0.3.2+2-canary




[v0.3.3]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.3.0...v0.3.3



[v0.3.4]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.3.3...v0.3.4


[v0.3.5-canary]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.3.4...v0.3.5-canary

[v0.3.5]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.3.4...v0.3.5
[v0.3.6]: https://github.com/TekExplorer/RixiansMSPain/compare/v0.3.5...v0.3.6

